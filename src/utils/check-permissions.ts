...
export async function checkUserPermission(
  ...
  const userPermissions = (await interaction.guild?.members.fetch({ user: interaction.user.id }))
    ?.permissions;
...
export async function checkBotPermission(
  ...
  const userPermissions = (
    await interaction.guild?.members.fetch({ user: interaction.client.user.id })
  )?.permissions;
